...
`make env` brings up dependencies via `docker compose` (see `Makefile` for the exact services). After containers are healthy, run the binary from the repo root with your config, for example:
...
| `queryflux-metrics` | Prometheus instrumentation, buffered & multi-store pipeline |
| `queryflux-auth` | Auth providers & authorization plumbing |
| `queryflux-fingerprint` | Query fingerprinting |

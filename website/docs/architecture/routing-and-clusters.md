...
- Runs off the async runtime (`spawn_blocking`) so a slow or GIL-bound script doesn't stall other in-flight queries; built-in strategies are unaffected and stay on the fast path.

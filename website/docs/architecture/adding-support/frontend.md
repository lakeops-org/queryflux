...
- **[`app/protocols/page.tsx`](https://github.com/lakeops-org/queryflux-studio/blob/main/app/protocols/page.tsx)** in the `queryflux-studio` repository — if you want an icon on the Protocols page, extend **`PROTOCOL_SIMPLE_ICONS`** keyed by the **`id`** your admin DTO uses (match **`build_frontends_status`**).

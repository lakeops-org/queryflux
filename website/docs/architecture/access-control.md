...
**Configuration today:** one or more named OPA connections (`accessControl.connections`, must include `"default"`), a global `enabled` default, and per–cluster-group overrides — `enabled`, `failOpen`, and which connection to use — under `accessControl.groups`. Edited on Studio **Access Control** (the `default` connection + scope), not on **Clusters**; additional named connections are YAML/Admin-API only. Policy (Rego) is never stored in QueryFlux. See [product model](../access-control/overview#product-model-what-queryflux-configures) and [multiple connections](../access-control/overview#multiple-connections).

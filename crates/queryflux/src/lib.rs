//! QueryFlux embedding library: construct a [`QueryFlux`] instance via
//! [`QueryFlux::builder`], then [`QueryFlux::serve`] it. The `queryflux` binary
//! is a thin consumer of this same builder — see `src/main.rs`.
...
/// Load `config_path`, run pending persistence migrations, and exit.
///
/// Mirrors the `queryflux migrate` CLI action; kept in the library so both the
/// shipped binary and embedders that want a programmatic migration step share
/// one implementation.
pub async fn run_migration(config: &str) -> Result<()> {
...
impl QueryFluxBuilder {
    /// Path to the YAML config file. Required before [`Self::build`].
    ...
    /// Load config, build the router/guard/adapter chain, and start background
    /// maintenance tasks (config hot-reload, health checks, capacity coordination).
    /// Does not start listening for client connections — call [`QueryFlux::serve`]
    /// for that.
    pub async fn build(self) -> Result<QueryFlux> {
            ...
            #[cfg(feature = "otlp")]
            {
                if let Some(endpoint) = &config.queryflux.otlp_endpoint {
                    ...
                    let telemetry = tracing_opentelemetry::layer()
                        .with_tracer(tracer_provider.tracer("queryflux"));
        ...
        let distributed_backend: Option<Arc<dyn DistributedBackendStore>> =
            pg_store.map(|pg| pg as _);
        ...
        if let Some(pg) = &backend {
            ...
            let mut clusters: HashMap<String, queryflux_core::config::ClusterConfig> =
                HashMap::new();
            for r in &db_cluster_records {
                ...
                let query_auth =
                    match queryflux_core::engine_registry::parse_query_auth_from_config_json(
                        &r.config,
                    ) {
                        Ok(qa) => qa,
                        Err(e) => {
                ...
                let auth =
                    match queryflux_core::engine_registry::parse_auth_from_config_json(&r.config) {
                        Ok(a) => a,
                        Err(e) => {
                            tracing::warn!(
                                cluster = %r.name,
        ...
        if let Some(records) = &startup_cluster_records {
            for record in records {
                ...
                let (health_check_query, reconcile_query) =
                    extract_base_probe_queries(&record.config);
            ...
            &startup_cluster_records
        {
            ...
            for r in records {
                ...
                if variants.is_empty() {
                    m.insert(
                        r.name.clone(),
                        serde_json::to_string(&(r.engine_key.as_str(), &r.config))
                            .unwrap_or_default(),
                    );
                } else {
                    let (health_check_query, reconcile_query) =
                        extract_base_probe_queries(&r.config);
                    match queryflux_core::config::expand_cluster_variants(
                        &r.name,
                        &r.config,
                        &r.engine_key,
            ...
            .cache_backend
        {
            Some(cache_cfg) => {
                ...
                match queryflux_cache::opendal_cache::OpenDalResultCache::new(
                    cache_cfg,
                    cache_store,
                ) {
                    Ok(c) => {
        ...
        tokio::spawn({
            ...
            async move {
                ...
                loop {
                    ...
                    let cutoff =
                        chrono::Utc::now() - chrono::Duration::seconds(CLIENT_TIMEOUT_SECS);
        ...
        tokio::spawn({
            ...
            async move {
                ...
                loop {
                    ...
                    let sweep_lock = match &distributed_backend {
                        Some(backend) => {
                            match backend.try_sweep_lock("stale-queued-eviction").await {
                                Ok(Some(lock)) => Some(lock),
                                Ok(None) => continue,
                                Err(e) => {
                                    state.metrics.on_coordination_failure("sweep_lock");
                                    tracing::warn!(
                                        "Stale queued sweep lock failed, sweeping anyway: {e}"
                                    );
                                    None
                                }
                            }
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    match backend.purge_old_query_records(cutoff).await {
                        ...
                        Ok(n) => {
                            tracing::info!(
                                "Purged {n} history rows older than {retention_days} days"
                            )
                        }
                                ...
                                for (adapter, cstate) in &targets {
                                    ...
                                    if let Some(count) = actual {
                                        if let Err(e) = capacity_store
                                            ...
                                            .await
                                        {
                                            state.metrics.on_coordination_failure(
                                                "engine_reconcile_publish",
                                            );
                                            tracing::warn!(
                                                cluster = %cluster_name,
...
/// A built QueryFlux runtime, ready to serve. Router/guard/adapter chains are live
/// and background maintenance tasks are already running; client frontends start
/// listening on [`Self::serve`].
pub struct QueryFlux {
...
impl QueryFlux {
    /// Start building a `QueryFlux` instance.
    pub fn builder() -> QueryFluxBuilder {
        QueryFluxBuilder::default()
    }

    /// Shared runtime state (routing, guards, adapters, persistence, metrics).
    /// Exposed for embedders and tests that need to dispatch queries or inspect
    /// live state without starting network listeners.
    pub fn app_state(&self) -> &Arc<AppState> {
        &self.app_state
    }

    /// Spawn all enabled frontends and the admin server, then block until a
    /// shutdown signal (SIGINT/SIGTERM) is received, draining in-flight requests
    /// before returning.
    pub async fn serve(self) -> Result<()> {
        ...
        let mut trino_handle = tokio::spawn({
            ...
            let cfg = config.queryflux.frontends.trino_http.clone();
            async move {
                if cfg.enabled {

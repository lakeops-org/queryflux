...
impl AdbcAdapter {
    pub fn new(
        ...
        let db_kwargs = config.db_kwargs;
        ...
        let mut opts: Vec<(OptionDatabase, adbc_core::options::OptionValue)> =
            vec![(OptionDatabase::Uri, uri.clone().into())];
        ...
        let introspection =
            build_introspection(&driver_name, &cluster_name, &uri, &db_kwargs, pool.clone());

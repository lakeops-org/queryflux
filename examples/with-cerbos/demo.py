...
def trino_query(user: str, password: str, sql: str) -> tuple[list[str] | None, list[list[object]], str | None]:
    ...
    headers = {
        "Authorization": _basic(user, password),
        "X-Trino-User": user,
        "X-Trino-Source": "cerbos-demo",
    }
...
def dry_run(user: str, groups: list[str], roles: list[str], sql: str) -> None:
    # Cerbos hard-requires a non-empty principal.roles (unlike OPA, which is free-form) —
    # an identity with none is denied locally without even calling Cerbos. See
    # CerbosProvider::evaluate's doc comment.
    payload = json.dumps(
        {
            "sql": sql,
            "clusterGroup": "trino-cerbos",
            "dialect": "trino",
            "identity": {"user": user, "groups": groups, "roles": roles},
        }
    ).encode()
...
def ask_cerbos(user: str, role: str, table: str) -> None:
    """Call Cerbos's CheckResources directly — the same shape CerbosProvider sends."""
    payload = json.dumps(
        {
            "requestId": "demo",
            "principal": {"id": user, "roles": [role], "attr": {}},
            "resources": [
                {
                    "resource": {"id": table, "kind": "table", "attr": {"table": table}},
    ...
    body = _request(
        f"{CERBOS}/api/check/resources",
        data=payload,
        headers={"Content-Type": "application/json"},
    )
    print(f"\n== Cerbos CheckResources as {user} ({role}) on {table}")
    print(json.dumps(body, indent=2))
...
def main() -> int:
    ...
    wait_http(f"{CERBOS}/_cerbos/health", "Cerbos")
    ...
    if args.dry_run_only:
        dry_run("bob", ["analysts"], ["analyst"], f"SELECT name, region, ssn FROM {CUSTOMERS}")
        dry_run("bob", ["analysts"], ["analyst"], f"SELECT * FROM {PAYROLL}")
        return 0
    ...
    dry_run("bob", ["analysts"], ["analyst"], f"SELECT name, region, ssn FROM {CUSTOMERS}")
    ask_cerbos("bob", "analyst", "customers")
    ask_cerbos("bob", "analyst", "payroll")
    return 0
